							...
							<table class="table table-striped">
								...
								<tbody> 
									<c:forEach items="${Teacher}" var="teacher">
										<tr>
										<td>${teacher.user_count  }</td>
										<td>${teacher.name  }</td>
										<td>${teacher.password  }</td>
										<td>
												<a href="teacher_modify.jsp">
														<button type="button" class="btn btn-sm btn-primary" id="modify" name="but" value="0">修改</button></a>
											<button type="button" class="btn btn-sm btn-primary" id="delete" name="but" value="1">删除</button>
